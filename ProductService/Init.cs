...
public static class Init
{
    ...
    private static void BuildServices(WebApplicationBuilder builder)
    {
        ...
        //自动注入WebApi服务接口
        builder.Services.AutoRegistryService(new List<string>() { "ProductService" });
        //添加SqlSugar服务
        builder.Services.AddSqlsugarSetup(builder.Configuration);
        DbConnection connection = new MySqlConnection(builder.Configuration.GetConnectionString("ProductContext"));
        //添加CAP，EventBus消息总线实现事件发布、订阅基于MQ实现
        builder.Services.AddDbContext<ProductContext>(opt => opt.UseMySql(connection,MySqlServerVersion.LatestSupportedServerVersion));
